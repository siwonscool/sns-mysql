...
## 5. 페이지네이션
### 오프셋 기반의 페이지네이션
![오프셋 기반의 페이지네이션 ui](README_IMG/pagination_ui.png)  
- 위와같은 페이지네이션을 구현하려면 오프셋 기반의 페이지네이션 이 필수적이다.
- 하지만 오프셋 기반의 페이지네이션은 마지막 페이지를 구하기위해 전체 개수를 알아야한다.  
- 데이터 용량이 많아지게되면 전체 데이터 개수를 구하는건 부하가 걸릴 수 있다.
<br><br>

![오프셋 기반의 페이지네이션](README_IMG/offset_pagination.png)
- 오프셋 기반의 페이지네이션은 오프셋 부터 시작하기 위해 오프셋 이전데이터까지 모두 다시 읽는다 (버리는 데이터가 많아진다)
<br>

### 커서 기반의 페이지네이션
![커서 기반의 페이지네이션](README_IMG/cursor_pagination.png)
- 커서 기반의 페이지네이션은 커서가 되는 key, size 를 요청하며 마지막 key 를 응답으로 받아 다음 페이지에 사용할 수 있다.
- 이는 데이터양이 많이 질수록 불필요한 데이터 조회없이 빠른 조회가 가능하다
- 하지만 전체갯수를 조회하지 않기때문에 오프셋 기반의 페이지네이션 ui 에는 구현이 어렵다
